// Важно!!! Делаем поиск по всему проекту 51temphash и заменяем на +1 к числу, заменяем везде
// 51temphash  затем только билдим проект!!!
...
function scripts() {
  return src([
    ...
    .pipe(concat('app51temphash.min.js'))
    .pipe(uglify())
    .pipe(dest('app/js/'))
...
function scriptsMenu() {
  return src(['app/js/humburger.js'])
    .pipe(concat('appMenu51temphash.min.js'))
    .pipe(uglify())
    .pipe(dest('app/js/'))
    .pipe(browserSync.stream());
}
...
function styles() {
  return src('app/sass/style.scss')
    .pipe(sass())
    .pipe(concat('style51temphash.min.css'))
...
function buildcopy() {
  return src(
    [
      'app/css/**/*.min.css',
      'app/**/*.php',
      'app/js/app51temphash.min.js',
      'app/js/appMenu51temphash.min.js',
      'app/images/dest/**/*',
      'app/font/**/*',
      'app/video/**/*',
      'app/html/**/*.html',

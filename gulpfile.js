// Важно!!! Делаем поиск по всему проекту 37temphash и заменяем на +1 к числу, заменяем везде
// 37temphash  затем только билдим проект!!!
...
function scripts() {
  return src([
    ...
    .pipe(concat('app37temphash.min.js'))
    .pipe(uglify())
    .pipe(dest('app/js/'))
...
function scriptsMenu() {
  return src(['app/js/humburger.js'])
    .pipe(concat('appMenu37temphash.min.js'))
    .pipe(uglify())
    .pipe(dest('app/js/'))
    .pipe(browserSync.stream());
}
...
function styles() {
  return src('app/sass/style.scss')
    .pipe(sass())
    .pipe(concat('style37temphash.min.css'))
...
function buildcopy() {
  return src(
    [
      'app/css/**/*.min.css',
      'app/**/*.php',
      'app/js/app37temphash.min.js',
      'app/js/appMenu37temphash.min.js',
      'app/images/dest/**/*',
      'app/font/**/*',
      'app/video/**/*',
      'app/html/**/*.html',
